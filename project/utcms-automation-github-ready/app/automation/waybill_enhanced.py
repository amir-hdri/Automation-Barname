...
class EnhancedWaybillManager:
    ...
    async def _goto_with_retry(
        self, url: str, wait_until: Optional[str] = None
    ) -> None:
    ...
    async def create_waybill_with_map(
        ...
        try:
            ...
            dest_result = await self.location_selector.select_location(
                data.get("destination", {}), origin=False
            )
    ...
    async def _ensure_waybill_form_page(self):
                ...
                try:
                    ...
                    if href:
                        await self._goto_with_retry(
                            urljoin(current_url, href), wait_until="domcontentloaded"
                        )
                    else:
                        await action.click()
        ...
        try:
            ...
            interesting = [
                ...
                if ("بارنامه" in item.get("text", ""))
                or ("Waybill" in item.get("href", ""))
            ]
            logger.info(
        ...
        for candidate_url in self._waybill_url_candidates():
            try:
                await self._goto_with_retry(
                    candidate_url, wait_until="domcontentloaded"
                )
                await asyncio.sleep(1.2)
                if await self._is_waybill_form_ready():
                    return
    ...
    async def _submit_waybill(self) -> Dict[str, Any]:
        ...
        if not submit_clicked:
            raise WaybillError(
                "ارسال فرم بارنامه انجام نشد (کلیک روی دکمه ثبت ناموفق بود)"
            )
