...
class UTCMSAuthenticator:
    ...
    def _is_login_url(self, url: str) -> bool:
        ...
        return any(
            fragment in lowered
            for fragment in ("/login", "/account/login", "/signin", "/sign-in")
        )

    def _candidate_login_urls(self) -> list[str]:
        base_url = utcms_config.BASE_URL.rstrip("/")
        candidates = [utcms_config.LOGIN_URL.strip()]
        candidates.extend(f"{base_url}{path}" for path in self.LOGIN_PATH_CANDIDATES)
    ...
    async def _looks_like_login_page(self) -> bool:
        ...
        username_selector = await self._find_selector(self.USERNAME_SELECTORS)
        password_selector = await self._find_selector(self.PASSWORD_SELECTORS)
        submit_selector = await self._find_selector(self.SUBMIT_SELECTORS)
    ...
    async def _is_logged_in(self) -> bool:
        ...
        try:
            await self._goto_with_retry(
                utcms_config.WAYBILL_URL, wait_until="domcontentloaded"
            )
        ...
        if self._is_login_url(current_url):
            return False
        if await self._find_selector(self.LOGOUT_SELECTORS, visible=True, timeout=500):
            return True

        for selector in self.WAYBILL_FORM_MARKERS:
            try:
    ...
    async def _extract_login_error(self) -> Optional[str]:
        for selector in self.LOGIN_ERROR_SELECTORS:
            try:
                element = await self.page.query_selector(selector)
                if not element:
                    continue
    ...
    async def _wait_for_login_result(self, timeout_ms: int = 12000) -> bool:
        ...
        while asyncio.get_running_loop().time() < deadline:
            if await self._find_selector(
                self.LOGOUT_SELECTORS, visible=True, timeout=300
            ):
                return True
        ...
        return not await self._looks_like_login_page() and not self._is_login_url(
    ...
    async def _solve_captcha_with_provider(self) -> Optional[str]:
        ...
        logger.warning(
            ...
            extra={
                "extra_fields": {"provider": result.provider, "error": result.error}
            },
        )
        return None
    ...
    async def login(self, username: str, password: str) -> bool:
        ...
        for login_url in self._candidate_login_urls():
            ...
            username_selector = await self._find_selector(
                self.USERNAME_SELECTORS, visible=True, timeout=4000
            )
            password_selector = await self._find_selector(
                self.PASSWORD_SELECTORS, visible=True, timeout=4000
            )
            submit_selector = await self._find_selector(
                self.SUBMIT_SELECTORS, visible=True, timeout=4000
            )
            ...
            captcha_selector = await self._find_selector(self.CAPTCHA_SELECTORS)
            if captcha_selector:
                if utcms_config.UTCMS_CAPTCHA_VALUE:
                    try:
                        await self.page.fill(
                            captcha_selector, utcms_config.UTCMS_CAPTCHA_VALUE
                        )
                    except Exception:
                    ...
                    allow_manual = (
                        captcha_mode in ("provider_first", "manual_only")
                        and utcms_config.UTCMS_ENABLE_MANUAL_CAPTCHA
                    )
                    ...
                    if solved_by_provider:
                        try:
                            ...
                            self.last_error = (
                                "حل کپچا موفق بود اما مقداردهی فیلد کپچا انجام نشد."
                            )
                            return False
                    elif allow_manual:
                        if utcms_config.HEADLESS:
                            ...
                            return False
                        solved = await self._wait_for_manual_captcha_input(
                            captcha_selector
                        )
                        if not solved:
            ...
            if await self._wait_for_login_result():
                ...
                if not self.last_error:
                    self.last_error = (
                        await self._extract_login_error()
                        or "لاگین تکمیل نشد و دسترسی به فرم بارنامه تایید نشد."
                    )
                continue
            ...
            if not self.last_error:
                self.last_error = (
                    await self._extract_login_error()
                    or "لاگین ناموفق بود؛ صفحه در وضعیت ورود باقی ماند."
                )
        ...
        logger.warning(
            "login_failed", extra={"extra_fields": {"reason": self.last_error}}
        )
        return False
