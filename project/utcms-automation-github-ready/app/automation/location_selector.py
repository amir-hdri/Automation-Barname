...
class LocationSelector:
    ...
    async def select_location(
        self, location_data: Dict[str, Any], origin: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        raise LocationSelectionError(f"انتخاب مکان با شکست مواجه شد: {location_data}")

    async def _try_map_selection(
        self, location_data: Dict[str, Any], prefix: str
    ) -> Dict[str, Any]:
    ...
    async def _try_dropdown_selection(
        ...
        try:
            ...
            province_selected = await self._select_from_options(
                province_selectors, location_data.get("province", "")
            )
            ...
            city_selected = await self._select_from_options(
                city_selectors, location_data.get("city", "")
            )
            ...
            await self._select_from_options(
                district_selectors, location_data.get("district", "")
            )
    ...
    async def _try_text_input(
        ...
        try:
            ...
            search_text = (
                f"{location_data.get('city', '')} {location_data.get('address', '')}"
            )
    ...
    async def _geocode_address(
        ...
        address = (
            f"{location_data.get('city', '')}, {location_data.get('address', '')}, Iran"
        )
