...
async def _goto_with_retry(
    page, url: str, wait_until: str = "domcontentloaded"
) -> None:
...
class WaybillService:
    async def create_waybill_with_map(
        self, request: WaybillMapRequest
    ) -> Dict[str, Any]:
        request_id = str(uuid.uuid4())
        mode = (
            request.operation_mode.value
            if isinstance(request.operation_mode, OperationMode)
            else str(request.operation_mode)
        )
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                async with waybill_traffic_controller.slot(mode=mode):
                    ...
                    if not await auth._is_logged_in():
                        ...
                        if not username or not password:
                            raise HTTPException(
                                status_code=401,
                                detail="اطلاعات ورود به سیستم تنظیم نشده است",
                            )
                    ...
                    await report_service.record_success(
                ...
                if is_temporary and attempt < max_attempts:
                    await waybill_traffic_controller.mark_temporary_block(
                        multiplier=2.0
                    )
                ...
                raise HTTPException(
                    status_code=500, detail="خطای داخلی سرور در ثبت بارنامه"
                )
    ...
    @staticmethod
    def _build_response(
        request_id: str, mode: str, manager_result: Dict[str, Any]
    ) -> Dict[str, Any]:
        response: Dict[str, Any] = {
            ...
            "status": manager_result.get(
                "status", "validated" if mode == "safe" else "submitted"
            ),
        }

        if mode == OperationMode.SAFE.value:
            response["validation_summary"] = manager_result.get(
                "validation_summary", {}
            )
        else:
