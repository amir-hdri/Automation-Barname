...
class TestBrowserManager(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_create_context_loads_saved_auth_state(self):
        ...
        with patch(
            "app.core.config.utcms_config.USE_PERSISTENT_AUTH_STATE", True
        ), patch(
            "app.core.config.utcms_config.AUTH_STATE_PATH", "/tmp/utcms_state.json"
        ), patch(
            "app.automation.browser.os.path.exists", return_value=True
        ):
