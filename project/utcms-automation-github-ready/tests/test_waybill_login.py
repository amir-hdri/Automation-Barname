...
async def test_waybill_login_success():
    ...
    with patch(
        "app.automation.browser.browser_manager.initialize", new_callable=AsyncMock
    ) as mock_init, patch(
        "app.automation.browser.browser_manager.create_context", new_callable=AsyncMock
    ) as mock_create_context, patch(
        "app.automation.browser.browser_manager.new_page", new_callable=AsyncMock
    ) as mock_new_page, patch(
        "app.automation.browser.browser_manager.close_context", new_callable=AsyncMock
    ) as mock_close_context, patch(
        "app.automation.auth.UTCMSAuthenticator"
    ) as MockAuth, patch(
        "app.automation.waybill_enhanced.EnhancedWaybillManager"
    ) as MockManager, patch(
        "app.automation.reporting.report_service.record_request", new_callable=AsyncMock
    ), patch(
        "app.automation.reporting.report_service.record_success", new_callable=AsyncMock
    ), patch(
        "app.automation.reporting.report_service.record_map_usage",
        new_callable=AsyncMock,
    ) as mock_record_map_usage, patch(
        "app.core.config.utcms_config.UTCMS_USERNAME", "testuser"
    ), patch(
        "app.core.config.utcms_config.UTCMS_PASSWORD", "testpass"
    ):
        ...
        mock_manager_instance.create_waybill_with_map = AsyncMock(
            return_value={"origin_method": "map"}
        )
...
async def test_waybill_login_failure():
    ...
    with patch(
        ...
        "app.automation.reporting.report_service.record_failure", new_callable=AsyncMock
    ), patch(
        "app.core.config.utcms_config.UTCMS_USERNAME", "testuser"
    ), patch(
        "app.core.config.utcms_config.UTCMS_PASSWORD", "testpass"
...
async def test_waybill_missing_credentials():
    ...
    with patch(
        ...
        "app.core.config.utcms_config.UTCMS_USERNAME", ""
    ), patch(
        "app.core.config.utcms_config.UTCMS_PASSWORD", ""
    ):
