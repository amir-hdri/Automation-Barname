...
async def test_twocaptcha_success_flow():
    provider = TwoCaptchaProvider(
        api_key="key", timeout_seconds=30, poll_seconds=0.1, max_retries=1
    )

    with patch.object(
        provider, "_create_task", AsyncMock(return_value="123")
    ), patch.object(provider, "_poll_result", AsyncMock(return_value="abcd")):
