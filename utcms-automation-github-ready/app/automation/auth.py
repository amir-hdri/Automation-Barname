...
class UTCMSAuthenticator:
    ...
    async def _wait_for_login_result(self, timeout_ms: int = 12000) -> bool:
        ...
        while asyncio.get_running_loop().time() < deadline:
            if await self._find_selector(self.LOGOUT_SELECTORS, visible=True, timeout=300):
                return True
    ...
    async def login(self, username: str, password: str) -> bool:
        ...
        for login_url in self._candidate_login_urls():
            ...
            username_selector = await self._find_selector(self.USERNAME_SELECTORS, visible=True, timeout=4000)
            password_selector = await self._find_selector(self.PASSWORD_SELECTORS, visible=True, timeout=4000)
            submit_selector = await self._find_selector(self.SUBMIT_SELECTORS, visible=True, timeout=4000)
