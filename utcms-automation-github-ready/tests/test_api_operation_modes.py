...
def test_api_defaults_to_safe_mode():
    async def fake_handler(request):
        return {"success": True, "mode": request.operation_mode.value, "status": "validated", "request_id": "rid"}

    with patch("app.core.config.utcms_config.API_AUTH_MODE", "off"), patch(
        "app.services.waybill_service.waybill_service.create_waybill_with_map", new=AsyncMock(side_effect=fake_handler)
    ):
